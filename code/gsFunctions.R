...
runCrossVal<-function(TrainTestData,modelType,grms,nrepeats,nfolds,ncores=1,
  ...
  fitModel<-possibly(function(splits,modelType,augmentTP,TrainTestData,GroupName,grms){
    ...
    randFormula<-paste0("~vsr(",gid,"a,Gu=A1)")
    if(modelType %in% c("AD","ADE")){
      randFormula<-paste0(randFormula,"+vsr(",gid,"d,Gu=D1)")
      if(modelType=="ADE"){
        randFormula<-paste0(randFormula,"+vsr(",gid,"ad,Gu=AD1)")
        #"+vs(",gid,"aa,Gu=AA1)",
        #"+vs(",gid,"ad,Gu=AD1)")
        #"+vs(",gid,"dd,Gu=DD1)")
      }
  ...
  ## Parallelize
  require(furrr); plan(multicore); options(mc.cores=ncores);
  cvsamples<-cvsamples %>%
